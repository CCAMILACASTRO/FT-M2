...
function buildToDo(todo, index) {  // todo : ToDo {description: '', complete: false}
  // Tu código acá:                // index : num

  let toDoShell = document.createElement('div') //creo el elemento 'div'
  toDoShell.className = 'toDoShell'  // asignar nombre de clase

  let toDoText = document.createElement('span') // creo el elemento 'span'
  toDoText.innerHTML = todo.description;  // le asigno el valor de la prop description al elemento toDoText
  toDoText.id = index // asigno 'id'
...
function buildToDos(toDos) { // [ToDo {description: '', complete: false}]
  // Tu código acá:

  let array = toDos.map(buildToDo) //guardo en una variable lo que retorne como nuevo array.
  return array; 
  /*[ <div class = 'toDoShell' >
          <span id= 0> holi </span>
      </div>
...
function displayToDos() {
  ...
  let resultado = buildToDos(toDoItems) //llamamos a la funcion
  // for(let i = 0; i < resultado.length; i++){  //iteramos sobre el resultado donde guardamos lo que devuelve la funcion buildToDos
  //   toDoContainer.appendChild(resultado[i])  //Le agregamos un hijo a cada elemento del resultado.
  // }

  resultado.forEach(element => toDoContainer.appendChild(element));
...
function addToDo() {
  ...
  let input = document.querySelector('#toDoInput') //tomamos el elemento que le tenemos que pasar por parametro a la nueva clase.
  
  if(input.value !== ''){  //Si el input no es un string vacio... (si el usuario no escribio nada)
    let todo = new ToDo(input.value) //le pasamos el valor del input
    toDoItems.push(todo) //agregamos al array el objeto creado
    input.value = '' //Cuando creamos la tarjetita el input donde escribio el usuario se limpia
    displayToDos()
  } 
}
